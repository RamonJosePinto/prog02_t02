...
public class ManipularImagem {
    ...
    public static String gravarImagemPerfil(String caminhoArquivo, String nomeArquivo, int largura, int altura) throws IOException {
        File arquivoOrigem = Paths.get(caminhoArquivo).toFile();

        String novoCaminho = BASE_CAMINHO_IMAGENS + "pessoa/" + nomeArquivo;
        File arquivoDestino = Paths.get(novoCaminho).toFile();

        Files.copy(arquivoOrigem.toPath(), arquivoDestino.toPath(), StandardCopyOption.REPLACE_EXISTING);
        BufferedImage imagem = redimensionarImagem(caminhoArquivo, largura, altura);
        ImageIO.write(imagem, "jpg", arquivoDestino);

        return novoCaminho;
    }

    public static String gravarImagemAlbum(String caminhoArquivo, String nomeArquivo, int largura, int altura) throws IOException {
        File arquivoOrigem = Paths.get(caminhoArquivo).toFile();

        String novoCaminho = BASE_CAMINHO_IMAGENS + "album/" + nomeArquivo;
        File arquivoDestino = Paths.get(novoCaminho).toFile();
    ...
    public static BufferedImage redimensionarImagem(String caminhoImagem, int largura, int altura) {
        ...
        if (novaImgLargura >= largura) {
            ...
            while (novaImgAltura > altura) {
                novaImgLargura = (double) (--altura);
                novaImgAltura = (novaImgLargura * imgProporcao);
            }
        } else if (novaImgAltura >= altura) {
            imgProporcao = (novaImgLargura / novaImgAltura);//calcula a proporção  
            novaImgAltura = (double) altura;
            ...
            while (novaImgLargura > largura) {
                novaImgAltura = (double) (--largura);
